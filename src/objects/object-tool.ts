import {Object as IObject, Object} from 'fabric/fabric-impl';
...
export class ObjectTool {
  ...
  setValues(values: ObjectOptions, obj?: IObject | null) {
    ...
    if (isSvgSticker(obj) && values.fill !== obj.fill) {
      const fallbackColor = state().objects.active.editableProps.fill;
      obj.forEachObject(path => path.set('fill', values.fill || fallbackColor));
    }
